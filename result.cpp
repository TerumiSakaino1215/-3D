...
// マクロ定義
//*****************************************************************************
#define	TEXTURE_RESULT		"data/TEXTURE/title.png"		// 読み込むテクスチャファイル名
#define	TEXTURE_RESULT_LOGO	"data/TEXTURE/Result.png"	// 読み込むテクスチャファイル名

#define	RESULT_LOGO_POS_X	(125)		// リザルトロゴの位置(X座標)
#define	RESULT_LOGO_POS_Y	(275)		// リザルトロゴの位置(Y座標)
#define	RESULT_LOGO_WIDTH	(400)		// リザルトロゴの幅
#define	RESULT_LOGO_HEIGHT	(150)		// リザルトロゴの高さ
...
D3DXVECTOR3				g_posResultBG;					// 背景の位置
...
void UpdateResult(void)
{
	...
	// 毎フレーム実行する処理
	g_posResultBG.x -= 10.0f;

	if (g_posResultBG.x <= -SCREEN_WIDTH)
	{
		g_posResultBG.x = 0.0f;
	}
...
void SetVertexResultBG(void)
{
	...
	// 頂点座標の設定
	pVtx[0].vtx = D3DXVECTOR3(0.0f, 0.0f, 0.0f) + g_posResultBG;
	pVtx[1].vtx = D3DXVECTOR3(SCREEN_WIDTH * 2, 0.0f, 0.0f) + g_posResultBG;
	pVtx[2].vtx = D3DXVECTOR3(0.0f, SCREEN_HEIGHT, 0.0f) + g_posResultBG;
	pVtx[3].vtx = D3DXVECTOR3(SCREEN_WIDTH * 2, SCREEN_HEIGHT, 0.0f) + g_posResultBG;
